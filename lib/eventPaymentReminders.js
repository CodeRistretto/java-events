...
async function claimReminder(booking, reminderKey, daysUntil) {
  const { data, error } = await supabaseAdmin
    .from("event_email_log")
    .insert({
      booking_id: booking.id,
      reminder_key: reminderKey,
      email_type: "PENDING_SHOPIFY_BALANCE_INVOICE",
      recipient: booking.email,
      metadata: { days_until_event: daysUntil, sending: true },
    })
    .select("id")
    .single();
...
export async function runEventPaymentReminders(today) {
  ...
  for (const booking of bookings || []) {
    ...
    try {
      ...
      claimId = await claimReminder(booking, reminderKey, daysUntil);
      if (!claimId) {
        report.skipped.push({
          bookingId: booking.id,
          reminderKey,
      ...
      const subject =
        daysUntil <= 3
          ...
          : `Tu evento Java se acerca · saldo pendiente ${mxn(
              summary.remainingCents
            )}`;

      const baseUrl = process.env.APP_BASE_URL || "http://localhost:3000";
      const message =
        `Hola ${booking.customer_name}. Tu evento Java Coffee Cart es el ${pretty(
          booking.event_date
        )}. ` +
        `Tu saldo pendiente es ${mxn(
          summary.remainingCents
        )} y vence el ${pretty(dueDate)}. ` +
