  ...
  try {
    ...
    if (!bookingId) {
      return Response.json(
        { success: false, error: "Falta el identificador del evento." },
        { status: 400 }
      );
    }
    ...
    if (!booking) {
      return Response.json(
        { success: false, error: "No encontramos este evento." },
        { status: 404 }
      );
    }
    ...
    const contractedBalance =
      booking.balance_cents !== null && booking.balance_cents !== undefined
    ...
    const vat =
      snapshot.vatCents !== undefined ? centsToMoney(snapshot.vatCents) : null;
    ...
    if (vatPercent !== null && vatPercent > 0) {
      ...
      depositSubtotal = Math.round((deposit / (1 + rate)) * 100) / 100;
      depositVat = Math.round((deposit - depositSubtotal) * 100) / 100;
    }
    ...
    return Response.json({
      ...
      items: (items || []).map((item) => ({
        ...
        lineTotal: centsToMoney(item.line_total_cents),
      })),
    ...
    return Response.json(
      {
        success: false,
        error: error.message || "No fue posible cargar el evento.",
      },
      { status: 500 }
    );
  }
